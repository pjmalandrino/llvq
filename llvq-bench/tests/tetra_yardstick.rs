//! The bench is the yardstick: `llvq_search::tetra` must decode every word to
...
use llvq_search::tetra::{Tetra, LINEAR_COLUMNS, TRIO, WORD_BITS};
...
fn the_two_decoders_agree_on_a_hundred_thousand_words() {
    ...
    for _ in 0..100_000 {
        ...
        assert_eq!(tetra.decode_trio_order(w), bench, "{w:#014x} in trio order");
        assert_eq!(tetra.decode(w), point_to_natural(&bench, &trellis.code.order), "{w:#014x} in natural order");
    }
}
...
fn the_tables_are_the_same_bytes() {
    ...
    assert_eq!(*tetra.order(), trellis.code.order);
    assert_eq!(&tetra.rows()[..], &table.rows[..]);
    assert_eq!(LINEAR_COLUMNS, BENCH_COLUMNS);
    for s in 0..GOLAY_STATES {
        assert_eq!(tetra.prefixes()[s], trellis.prefixes[s], "prefixes of state {s}");
        assert_eq!(tetra.suffixes()[s], trellis.suffixes[s], "suffixes of state {s}");
        for b in 0..BRANCHES {
            assert_eq!(tetra.branches()[s][b], trellis.branches[s][b], "branch {b} of state {s}");
        }
    }
}
