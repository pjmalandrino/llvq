//! The tools that read an index as a v1 class refuse a Trio file by name.
//!
...
//! Trio word (format v5, kind `Trio`) names no class, and each of them would
...
//!
//! Each tool is run as the binary it is, on a two-block Trio file written
...
/// A one-matrix v5 Trio file, two blocks, on disk.
fn trio_file(name: &str) -> PathBuf {
    let trio = Trio::new();
...
fn refuses(bin: &str, args: &[&str], path: &std::path::Path) {
    ...
    assert!(!out.status.success(), "{bin} accepted a Trio file:\n{stderr}");
    ...
    assert!(stderr.contains("Trio"), "{bin}: the refusal must name the kind:\n{stderr}");
}

#[test]
fn rtbits_refuses_a_trio_file_by_name() {
    let p = trio_file("trio-rtbits.llvq");
    refuses(env!("CARGO_BIN_EXE_rtbits"), &[p.to_str().unwrap()], &p);
}

#[test]
fn classhist_refuses_a_trio_file_by_name() {
    let p = trio_file("trio-classhist.llvq");
    refuses(env!("CARGO_BIN_EXE_classhist"), &[p.to_str().unwrap()], &p);
}

#[test]
fn lswap_refuses_a_trio_file_by_name() {
    let p = trio_file("trio-lswap.llvq");
...
#[test]
fn radixstudy_refuses_a_trio_file_by_name() {
    let p = trio_file("trio-radixstudy.llvq");
