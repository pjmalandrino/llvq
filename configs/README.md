...
has a file of its own anyway, for the two things a run prints from it: the
note on the `served config:` line, and the path on every dump's `# config=`
line. Borrowing the 4B file would record the 4B as the object of an 8B run.
