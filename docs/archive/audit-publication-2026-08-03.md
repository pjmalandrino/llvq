...
### B1 — `docs/archive/mail-qualcomm-draft.md:25-29` : les chiffres rétractés
...
> réelle (`docs/archive/retraction-et-gain.md:77`) ; c'est son **étiquette de débit**
> qui était fausse. L'objet mesuré tournait à 2,7338 b/poids.

### B2 — `docs/archive/mail-qualcomm-draft.md:31-33` : l'écart au débit
...
### B3 — `docs/archive/mail-qualcomm-draft.md:38-44` : la question sur l'annexe G repose sur trois défauts empilés
...
### B4 — `docs/archive/mail-qualcomm-draft.md:50-57` : le noyau est présenté au futur
...
### B5 — `docs/archive/mail-qualcomm-draft.md:20-21` et `README.md:117` : « Spherical GPTQ is all there » est faux pour la config livrée
...
`docs/archive/retraction-et-gain.md:174-177` le dit déjà : « pour `LeechShapeGain`, la
...
`docs/archive/retraction-et-gain.md:145-151` : deux runs réputés identiques ont rendu
...
- « quantifier le gain ne coûte presque rien, 0,04 % de perplexité pour
  0,52 bit » — `docs/archive/retraction-et-gain.md:141-144` : « Elle n'a jamais été
...
`docs/archive/face-au-4-bits.md:24-31` — MLX q4 sur le même 4B, même machine, même
...
| D10 | `README.md:160`, `LAUNCH_ME.md:135` | « ~3,3 Go chargés » décrit `Slot32/Grouped32`, pas le runner livré : `docs/archive/pistes-battre-q4.md:69` donne **7,3 Go de pic** pour `bin/run`, et `LAUNCH_ME.md:55` exige « ~8 Go de RAM libre » — le même fichier se contredit |
...
| D15 | `docs/archive/g6-handover.md:15,21-25` ; `docs/archive/run-de-nuit.md:22,28` ; `docs/archive/passation-2026-07-31.md:8,15,17` ; `docs/archive/plan-mmlu.md` | Bandeau de péremption daté ou `docs/archive/`. Ces quatre documents nient l'artefact, le noyau et/ou le run MMLU |
| D16 | `docs/archive/retraction-et-gain.md:93` | La ligne attribue **15,3272** à `leech1c12`, la config du fichier publié. `llvq-llm/src/sealed.rs:11-16` explique que c'est l'overlay du **premier** run ; le fichier scellé donne 16,9617. Annoter |
...
| D19 | `docs/archive/pistes-battre-q4.md:95, 128-131` | P20 ✅ avec le résultat réel (×1,267 à 2,0436, 11,48 $) |
...
1. **Durée de quantification du 4B** : `CLAUDE.md:661` dit 3,45 h (mais c'est le
   ...
   `docs/archive/retraction-et-gain.md:86` donne 12 715 s pour le run de nuit.
