...
> 🗓️ **BANDEAU D'ÉTAT — dernière revue le 2026-08-08. Le portage est fait, mesuré, branché, et deux fois dépassé.** Cette spécification décrit le portage de `Slot32` ; le layout de production est désormais **`Planes14`** (4,804 b/poids, 2,14× vs FP16, **1,14× plus rapide que `Slot32` à contenu décodé identique**), avec `Planes12x` mesuré non branché (4,342) et `Golay70` mesuré et écarté (3,589, 1,31×). Le noyau tourne dans le modèle via `fused_cuda` + `bin/fusedrun` : **48,7 tok/s dans 2,96 Go contre 43,6 dans 8,04**, mêmes tokens gloutons jusqu'au token 89.
> **La phrase de statut ci-dessous — « aucune mesure CUDA n'existe » — est donc périmée**, et il en existe aujourd'hui une dizaine : [`mesures/cuda-preflight-2026-08-05.txt`](../mesures/cuda-preflight-2026-08-05.txt), [`mesures/cuda-matvec-2026-08-05.txt`](../mesures/cuda-matvec-2026-08-05.txt), [`mesures/rotation-cuda-2026-08-05.txt`](../mesures/rotation-cuda-2026-08-05.txt), [`mesures/a3-graph-2026-08-06.txt`](../mesures/a3-graph-2026-08-06.txt), [`mesures/c1-planesbench-2026-08-06.txt`](../mesures/c1-planesbench-2026-08-06.txt), [`mesures/planes14-fusedrun-2026-08-06.txt`](../mesures/planes14-fusedrun-2026-08-06.txt), [`mesures/e2-golay70-bench-2026-08-07.txt`](../mesures/e2-golay70-bench-2026-08-07.txt), [`mesures/phases-2026-08-07.txt`](../mesures/phases-2026-08-07.txt). Le document reste utile pour ses **dérivations** (§1 le format, §2 l'hôte, §4 le protocole) et pour la généalogie de ses prédictions ; ses **statuts**, non.
> ⚠️ Une prédiction structurante de ce document a été **infirmée par la mesure** : le conflit de bancs de §3.2 n'existe pas sur Apple, et le pas de 28 flottants ne gagne rien. Une autre a été **confirmée** : ε, le coût de lancement, vaut 3,63 µs — la fourchette [1 ; 4] µs que §6.3 refusait de trancher est fermée par le haut.

**Cible : NVIDIA L40S (sm_89, Ada, 48 Go). Périmètre : le chemin de production `Slot32` de `bin/thesis`. Statut *à la date de rédaction (2026-08-05)* : aucune mesure CUDA n'existe — tout chiffre de vitesse *CUDA* dans ce document est une arithmétique de structure, jamais une performance. Les chiffres Metal du lot K−1 (2026-08-05, §3.2, §3.7, §4.6bis, §6.1) sont, eux, des mesures, et deux d'entre elles contredisent ce que cette spécification prédisait : le padding de §3.2 ne gagne rien, et le protocole de §4.6 ne suffit pas.**
...
1. **La reproductibilité par un tiers.** `docs/archive/plan-de-test-v2-cuda.md` §4 classe le gate G6 en non-prouvable n° 6 : « le 2,07× est Metal et ne peut être reproduit sur cette cible par personne ». Un lecteur du papier n'a pas de Mac ; il a un job GPU. Le port transforme un chiffre invérifiable en un chiffre rejouable pour ~0,30 $.
...
`docs/archive/plan-de-test-v2-cuda.md` §4.4 posait une règle explicite : faire tourner `matvec_g32` et `matvec_flat` sur le **modèle entier** en Metal — les shaders existent (`llvq-metal/src/bin/matvec.rs`), ~1 h de dev, 3 min de run, 0 $. Cela referme le trou de `fiche-4b.md` §6.7 (« Grouped32 et Flat32 n'ont jamais tourné sur le modèle entier ») et donne la courbe bits↔vitesse **sur un seul protocole et un seul objet**, au lieu de mélanger les chiffres d'une couche isolée (`gate_proj`) et ceux du modèle entier.
