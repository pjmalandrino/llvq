...
Ce document **remplace** le plan v1 (`docs/archive/plan-de-test-papier.md`) sur trois points : la cible matérielle, le bras adverse, et l'axe vitesse. Il **reprend** le reste en le citant.
...
> ⚠️ Les lignes `L≤4` (4,667 b/poids) et `L≤3` (3,667) de `docs/archive/face-au-4-bits.md` § « La sortie n° 2, mesurée : plafonner les niveaux » (la table `lcap`) sont dérivées d'un **banc sur source gaussienne**, et le même banc donne 5,667 pour L=5 là où `format-noyau.md` § « Le prix en RAM, et que c'est un cadran » mesure **5,51** sur le 4B entier — il est 2,8 % haut. **Ne pas les mettre dans cette table sans étiquette.** L'énoncé de §7.2 doit reposer sur `Grouped32` (mesuré), pas sur `L≤3`.
...
- **~640 Ko/token** (`docs/archive/pistes-battre-q4.md:97`) = 2 × 327 680 : le chiffre 70B avec le facteur K/V compté **deux fois**. Faux dans toutes les lectures.
