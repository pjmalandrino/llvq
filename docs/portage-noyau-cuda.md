...
**Cible : NVIDIA L40S (sm_89, Ada, 48 Go). Périmètre : le chemin de production `Slot32` de `bin/thesis`. Statut : aucune mesure CUDA n'existe — tout chiffre de vitesse dans ce document est une arithmétique de structure, jamais une performance.**
...
Le nôtre existe. Il bat le FP16 de **2,06–2,08×** sur les 252 projections du modèle entier (`docs/fiche-4b.md` §6.4), vérifié sur 1 105 920 lignes contre une référence f64 avant toute mesure. Il tourne sur du Metal.
...
### 0.4 Le jalon gratuit qui doit précéder la décision

`docs/plan-de-test-v2-cuda.md` §4.4 pose une règle explicite : faire tourner `matvec_g32` et `matvec_flat` sur le **modèle entier** en Metal — les shaders existent (`llvq-metal/src/bin/matvec.rs`), ~1 h de dev, 3 min de run, 0 $. Cela referme le trou de `fiche-4b.md` §6.7 (« Grouped32 et Flat32 n'ont jamais tourné sur le modèle entier ») et donne la courbe bits↔vitesse **sur un seul protocole et un seul objet**, au lieu de mélanger 0,68×/0,90× (gate_proj, best-of-15, surcoût soustrait) et 2,07× (modèle entier).

S'y ajoute un second jalon local à 0 $, identifié par ce travail de spécification : **tester le padding de tuile contre les conflits de bancs sur Metal** (§3.2). Si le noyau Metal paie déjà le conflit, le correctif améliore le **2,07× publié**, et l'exercice de portage se rembourse avant d'avoir commencé.
...
- `Cargo.lock` **est suivi par git** (`git ls-files --error-unmatch Cargo.lock` réussit) et **est dans les `allow_patterns`** de la publication du Space (`ops/run.py:480`). La ligne `Cargo.lock` du `.gitignore` est **inerte** (un fichier suivi n'est pas ignoré) mais devrait être retirée par hygiène.
- **Ce qui manque réellement : `--locked`.** `ops/Dockerfile.cuda:48` fait `cargo build --release -p llvq-llm --features ${FEATURES}` sans `--locked` (vérifié par grep). Un noyau CUDA chronométré sur une résolution de dépendances non figée n'est pas rejouable. **C'est une ligne, pas un ticket.**
...
Le pas de 28 avec chargements `float4` explicites est la seule combinaison qui supprime le conflit **sans** perdre l'alignement 16 octets. La boucle interne de `slot_dot` est déjà structurée en `i += 4, k = 0..3` : le `float4` est naturel. Le coût : +2 048 octets de mémoire partagée par bloc (6 blocs/SM × 14,3 Ko = 86 Ko ≤ 100 Ko sur Ada, et l'occupancy reste plafonnée par les 1 536 threads/SM), plus une modification de la boucle de remplissage qui devient un scatter `xs[(i/24)*28 + i%24]` — **une division entière par thread et par itération, pas « une ligne »**.

Le bras FP16 a la même arithmétique en plus doux : `4L mod 32` → 8 bancs, 4 voies en scalaire, **zéro** avec un `float4` explicite et **sans padding**. Traiter les deux bras ou aucun ; corriger un seul truquerait le rapport.

**L'ampleur du gain est inconnue et le restera jusqu'à mesure.** Ce qui est décidable maintenant, c'est de tester le correctif **en Metal d'abord, gratuitement, sur le Mac** : si les 12 µs de décodage au-dessus du sol (`format-noyau.md:289`) bougent, c'est un gain acquis avant d'avoir dépensé un dollar, et ça informe le port.
...
`stride` vaut 11, 14 ou **17 octets** ; les blocs à 5 niveaux (3,4 % du total, `format-noyau.md:297-300`) fixent ~2/3 des groupes à 17. Un warp lisant un enregistrement par lane couvre 31×17 + 20 = 547 octets contigus, mais chaque lane lit 20 octets qui se chevauchent : **~17-18 secteurs de 32 o par instruction de chargement, là où l'idéal coalescé en fait 4**. Le trafic DRAM reste juste (le L1 absorbe les relectures) ; c'est le coût d'**émission LSU** qui est multiplié.
...
**Chiffre à corriger dans `format-noyau.md`** : le document annonce « ~4,4 b/poids » pour un plafonnement à L ≤ 4. L'arithmétique du format dit autre chose. `format-noyau.md:128` mesure exhaustivement 4 niveaux pour **65,9 %** des blocs ; après plafonnement cette part monte à 68,2 %, et la probabilité qu'un groupe de 32 ne contienne aucun bloc L = 4 est ~10⁻¹⁶ : **tous** les groupes prennent 14 octets. D'où 14×8/24 = **4,667** de payload + 0,042 d'adressage = **4,708 b/poids**. La prime de coalescence de l'option (c) est donc de **0,625 b/poids (13 %)**, pas de 0,93. Vérification disponible localement pour 0 $ et 6,5 s : `bin/rtbits` imprime déjà la moyenne des max par groupe.
...
| 7 passes, reps 0 et 1 jetées, **minimum** des 5 | identique — le bruit d'un GPU partagé est au-dessus, jamais en dessous |
| FP16 mesuré en premier | **entrelacer A,B,A,B,…** (voir §4.8) |
...
Le piège documenté (`format-noyau.md:242-251`) : des tampons de 11-17 Mo tenaient dans le SLC de 48 Mo du M3 Max et étaient rejoués 576 fois, rendant tous les chiffres LLVQ antérieurs optimistes.
...
- **L'image ne sait construire que quatre binaires** (`ops/Dockerfile.cuda:48-49`, `-p llvq-llm --bin smoke --bin ppl --bin oracle --bin mmlu`, copiés :66-71). Le banc se construit avec `-p llvq-cuda` : crate léger, aucun lien candle, donc **il n'aggrave pas** le risque SIGKILL documenté :31-43. Prévoir tout de même 2 tentatives de build, 40-70 min chacune, non facturées.
...
### K−1 — les deux jalons Metal, locaux, 0 $ (à faire avant d'engager)
...
**Pourquoi d'abord** : (a) répond en 1 h à la question que le port coûterait des semaines à poser ; (b) peut améliorer le 2,07× publié avant qu'un dollar soit dépensé ; (c) tranche la prime de coalescence de §3.7.
**Effort** : ~0,5 j. **Coût** : 0 $.

### K0 — le crate partagé `llvq-kernel`

**Livrable** : `f16_bits`, `f16_to_f64`, `GpuClassRec`, `gpu_class_table`, le chargement/transcodage des 252 matrices, la référence f64, `worst_error`, sortis de `llvq-metal`. `bin/thesis` rejoué avant/après doit rendre **le même rapport 2,06-2,08× et la même pire erreur 3,4·10⁻⁸**.
...
**Livrable** : `llvq-cuda` déclaré, coquille vide hors Linux, cudarc sous `[target.'cfg(target_os = "linux")'.dependencies]`, `cargo clippy --all-targets` **vert sur le Mac**. Plus `--locked` dans `ops/Dockerfile.cuda:48` et la ligne inerte retirée du `.gitignore`.
**Effort** : 0,25 j.
...
**Livrable** : les 252 matrices, prologue parallélisé, **vérification des 1 105 920 lignes contre la référence f64 avant toute mesure**, trois bras (cuBLAS, `tv_f16`, `tv_slot`) plus le noyau « sol », un stream, événements, entrelacement A/B, best-of-5-sur-7, ligne de résultat par variante de noyau, sha256 de la source compilée, relevé d'horloges.
**Effort** : 1,5-2,5 j.
...
**Balayage, dans cet ordre** : padding de tuile (§3.2) → `maxrregcount` (§3.9) → staging coopératif (§3.7-a) → CUDA Graph (§3.8) → taille de tuile.
...
### 6.1 Répondu localement, 0 $, avant tout engagement

| question | comment |
|---|---|
| Le noyau Metal paie-t-il déjà le conflit de bancs 8 voies ? | K−1(b) : padding + `float4`, rejeu de `thesis` |
| Que valent Grouped32 et Flat32 sur le modèle entier ? | K−1(a) : les shaders existent |
| Quelle est la vraie moyenne des strides sous plafond L ≤ 4 ? | K−1(c) : `rtbits`, 6,5 s |
