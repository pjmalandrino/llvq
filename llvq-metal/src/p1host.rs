...
/// The block record and the E1v addressing table moved to [`llvq_artifact::blockrec`]
/// when a second GPU needed them; re-exported here so every caller of this
/// module reads exactly as it did. See that module's header for why.
pub use llvq_artifact::blockrec::{
    binom_table, block_records, ceil_log2, e1v_fixture, e1v_payload_bits, golay_offsets,
    GpuBlockRec, BINOM_STRIDE, BLOCK_ENTRIES, E1V_PAY_ENTRIES,
};
pub use llvq_artifact::e1v::{E1V_GROUP, E1V_ORIGIN_ID};

use llvq_core::{Golay, DIM};
use llvq_search::fastdec::{FastDecoder, MAX_KINDS};
use llvq_search::cns::CnsRecord;
...
// ---------------------------------------------------------------------------
// P1c — the E1v stream's addressing table
// ---------------------------------------------------------------------------
...
/// The **CNS's** own point, dotted in f64 — the etalon P1c names for the
...
/// equality re-establishes C2 on the very blocks being timed, for the cost of
/// one CPU pass.
///
/// The scale is rebuilt from the decoded point through
/// [`llvq_core::Leech::shell_index`], exactly as [`etalon_cascade`] does, so a
/// wrong norm in a host table cannot cancel against itself here either. The
/// origin decodes to the zero vector, whose shell is zero and whose dot is zero
