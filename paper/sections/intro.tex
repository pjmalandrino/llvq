...
of payload; the bytes actually written are 2.17 over the quantized weights
alone, the $+8\%$ of \S\ref{sec:evaluation}) but the rate of whatever
format the kernel reads. Our
...
rests on scale, and two further models measure it. The degradation falls ---
...
over FP16 loses 42.8\% of itself on the first step and 13.9\% on the
...
unresolved on the MMLU gap, likely a matter of statistical power --- and we
carry the two verdicts separately throughout
(\S\ref{sec:evaluation}, Appendix~\ref{app:scale}). Three points, no law.
On memory the three points do agree: the served model is below the 4-bit
