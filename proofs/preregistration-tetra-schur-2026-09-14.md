# Tetra gain choices and conditional continuation

The operator approved this local pilot and timestamping on 2026-09-14.
Approval covers both seeds, the checkpoint-prefix scope and the 30-minute total ceiling.
...
Descriptive per-seed summaries weight the six projection-depth cells equally; no pooled confidence interval is inferred.
...
Time estimate: 5–15 minutes total for both seeds, with a 30-minute ceiling (*estimated*, not measured).
...
The authorized supervisor caps capture plus replay at 1,800 wall-clock seconds.
Processes start at nice 10, with LLVQ_THREADS=12 and RAYON_NUM_THREADS=12 on this 16-thread Mac.
The settled SHA-256 manifest is `tetra-schur-inputs-2026-09-14.sha256` beside this preregistration.
Both files must carry matching OpenTimestamps receipts before the supervisor starts.
