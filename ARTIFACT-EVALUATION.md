...
**This table read 63 / 10 / 55 / 19+13 until 2026-08-25, and every one of
...
**The repository stays public for the duration of the review, operator
...
2. **The published bytes are not byte-reproducible, and we say so in the
   ...
   **What this does *not* loosen, and the distinction is the whole point.**
   ...
   not carry this variance. (c) The repository's inherited working rule,
...
3. **Two of the three model sizes are not published.** The 8B and 14B sealed
   ...
   **The 8B, however, no longer needs re-quantizing, it needs re-sealing,
   ...
   (`docs/mesures/b3-8b-reseal-2026-08-18.txt`). What that does *not*
...
`scripts/make_figures.py` regenerates every figure from the committed CSVs, and
...
- **Decision thresholds are pre-registered.** `proofs/` holds the documents
  ...
  documents and 16 `.ots` anchor files**. Those two counts do not pair off
...
*This line read "\$63.36 across 55 jobs" until 2026-08-25; the ledger had
...
**The last row is the one to read before budgeting a reproduction.** It buys
